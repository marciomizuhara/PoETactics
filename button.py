...
class SoulBox():
	def __init__(self, image, soul_icon, sphere, pos, skill, description, soul_cost, skill_font, description_font, soul_cost_font, base_color, hovering_color):
		self.image = image
		...
		if self.sphere:
			self.sphere = GRAY_SPHERE
		else:
			self.sphere = GREEN_SPHERE
		self.base_color, self.hovering_color = base_color, hovering_color
		...
		self.rect = self.image.get_rect(center=(self.x_pos, self.y_pos))
		self.skill_text_rect = self.image.get_rect(midleft=(self.x_pos - 280, self.y_pos + 12))
		self.description_text_rect = self.image.get_rect(midleft=(self.x_pos-80, self.y_pos + 16))
		...
		self.soul_cost_text_rect.left = self.rect.right - soul_cost_text_rect_deviation.right - 60  # Ajuste o valor 10 para a margem desejada
		self.soul_cost_text_rect.top = self.rect.top + 10
		self.soul_icon_rect = self.image.get_rect(midleft=(self.x_pos-340, self.y_pos+5))
		self.sphere_rect_deviation = self.image.get_rect(center=(self.x_pos, self.y_pos))
		self.sphere_rect_deviation.top = self.rect.top + 10
		self.sphere_rect_deviation.left = self.rect.right - 40
	...
	def update(self, screen):
		...
		screen.blit(self.sphere, self.sphere_rect_deviation)
